    ...
    public function run()
    {
        ...
        if (!$dataType->exists) {
            $dataType->fill([
                ...
                'policy_name'           => null,
                'controller'            => '\App\Http\Controllers\Admin\ProductsController',
        ...
        if (!$dataType->exists) {
            $dataType->fill([
                ...
                'model_name'            => 'App\Model\CategoryProduct',
                'controller'            => '',
                'generate_permissions'  => 1,
                'description'           => '',
            ])->save();
        ...
        if (!$dataType->exists) {
            $dataType->fill([
                ...
                'controller' => '\\App\\Http\\Controllers\\Admin\\OrdersController',
                'generate_permissions' => 1,
                'description' => '',
            ])->save();
        }

        $dataType = $this->dataType('name', 'stores');
        if (!$dataType->exists) {
            $dataType->fill([
                'slug' => 'stores',
                ...
                'icon' => 'voyager-documentation',
                'model_name' => 'App\\Model\\Store',
                'controller' => '\\App\\Http\\Controllers\\Admin\\StoresController',
                'generate_permissions' => 1,
                'description' => '',
            ])->save();
        }
