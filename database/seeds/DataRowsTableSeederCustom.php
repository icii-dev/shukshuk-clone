...
class DataRowsTableSeederCustom extends Seeder
{
    ...
    public function run()
    {
        ...
        */
        ...
        $dataRow = $this->dataRow($productDataType, 'product_belongsto_store_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
                'display_name' => 'Stores',
                ...
                'add'          => 1,
                'delete'       => 1,
                'details'      => json_decode("{\"model\":\"App\\\\Model\\\\Store\",\"table\":\"stores\",\"type\":\"belongsTo\",\"column\":\"store_id\",\"key\":\"id\",\"label\":\"name\",\"pivot_table\":\"address_districts_indo\",\"pivot\":\"0\",\"taggable\":null}"),
                'order'        => 7,
            ])->save();
        }
