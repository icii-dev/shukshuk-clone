                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->field=='featured')
                                                    @if($data->{$row->field})
                                                        <a href="{{route('admin.stores.featured', [$data->{$data->getKeyName()}, \App\Model\Product::FEATURED_OFF])}}"><span class="label label-info">DElETE Featured</span></a>
                                                    @else
                                                        <a href="{{route('admin.stores.featured', [$data->{$data->getKeyName()}, \App\Model\Product::FEATURED_ON])}}"><span class="label label-primary">ADD Featured</span></a>
                                                    @endif
                                                @elseif($row->type == 'image')
                                                    <img src="@if( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ url( STORE_AVATAR_PATH.strval($data->{$row->field}) ) }}@else {{ $data->{$row->field} }}@endif" style="width:100px" onerror="this.src='{{asset('img/store-avatar/default-avatar.png')}}'">
                                        ...
                                        <td class="no-sort no-click" id="bread-actions">
                                            @can('edit', $data)
                                                <a href="{{ route('voyager.'.$dataType->slug.'.edit', $data->{$data->getKeyName()}) }}" title="Review" class="btn btn-sm btn-primary pull-right edit">
                                                    <i class="voyager-edit"></i> <span class="hidden-xs hidden-sm">Review</span>
                                                </a>
                                            @endcan
