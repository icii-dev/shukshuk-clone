...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-news"></i> Components
    </h1>
@stop
...
@section('content')
    ...
    <div class="page-content settings container-fluid">
        <form action="{{ route('voyager.components.update', '1') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
                        ...
                        <div id="{{ \Illuminate\Support\Str::slug($group) }}" class="tab-pane fade in @if($group == $active) active @endif">
                            ...
                            <div class="panel-heading">
                                ...
                                <div class="panel-actions">
                                    <a href="{{ route('admin.components.move_up', $setting->id) }}">
                                        <i class="sort-icons voyager-sort-asc"></i>
                                    </a>
                                    <a href="{{ route('admin.components.move_down', $setting->id) }}">
...
{{--                                        <input type="text" class="form-control" name="{{ $setting->key }}" value="{{ $setting->value }}">--}}
                                        ...
                                        <div class="form-group">
                                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('voyager.storage.disk'))->exists($setting->value))
                                                <div class="img_settings_container">
                                                    <a href="{{ route('admin.components.delete_value', $setting->id) }}" class="voyager-x delete_value"></a>
                                    ...
                                    @if($group === 'Banner')
                                        <div class="form-group">
                                            <input type="text" class="form-control" name="{{ $setting->key }}__display_name" value="{{ $setting->display_name }}">
                                        </div>
                                        <div class="form-group">
                                            <input type="text" class="form-control" name="{{ $setting->key }}__details" value="{{ $setting->details }}">
                                        </div>
                                    @endif
                                </div>
        ...
        <div class="panel" style="margin-top:10px;">
            ...
            <div class="panel-body">
                <form action="{{ route('voyager.components.store') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="setting_tab" class="setting_tab" value="{{ $active }}" />
                    <div class="col-md-3">
    ...
    <script>
        $('document').ready(function () {
            ...
            $('.panel-actions .voyager-trash').click(function () {
                ...
                $('#delete_form')[0].action = '{{ route('admin.components.delete', [ 'id' => '__id' ]) }}'.replace('__id', $(this).data('id'));
